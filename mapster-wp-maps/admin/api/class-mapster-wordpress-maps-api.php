        ...
        function mapster_wp_maps_get_features($params) {
            ...
            if(mapster_can_be_looped($catsArray)) {
              if(count($catsArray) > 0) {
                $args = array(
                  'post_type' => array('mapster-wp-user-sub', 'mapster-wp-location', 'mapster-wp-polygon', 'mapster-wp-line'),
                  'tax_query' => array(
                    array(
                      "taxonomy" => "wp-map-category",
                      "field" => "term_id",
        ...
        function mapster_wp_maps_get_single_map($params) {
            ...
            if ( mwm_fs()->is__premium_only() ) {
              if( mwm_fs()->can_use_premium_code() ) {
                $progressive_map = get_field("progressive_loading_enabled", $post_id, true);
              }
            }
            ...
            if($single_feature_id || $feature_ids) {
              ...
              foreach($features_to_fetch as $single_feature) {
                ...
                } else if($single_feature_post_type === 'mapster-wp-line') {
                  array_push($minimized_line_data, $dataToAdd);
                } else if($single_feature_post_type === 'mapster-wp-polygon') {
                  array_push($minimized_polygon_data, $dataToAdd);
                }
              }
            } else {
              // Check if this has a cache specified -- if so, just immediately return that cache
          	  if ( mwm_fs()->is__premium_only() ) {
          			if( mwm_fs()->can_use_premium_code() ) {
                  $cache_enabled = get_field('cache_use_cache', $post_id);
                  if($cache_enabled && $ignore_cache !== "1") {
                    $cached_file = trailingslashit(wp_upload_dir()['basedir']) . 'mapster/map-' . $post_id . '.json';
                    if(file_exists($cached_file)) {
                      return json_decode(file_get_contents($cached_file));
                    }
                  }
                }
              }
              ...
              if($progressive_map) {
                ...
                // Try adding any extra added posts or pages
            	  if ( mwm_fs()->is__premium_only() ) {
            			if( mwm_fs()->can_use_premium_code() ) {
                    if(isset($minimized_data['add_custom_posts']) && mapster_can_be_looped($minimized_data['add_custom_posts'])) {
                      foreach($minimized_data['add_custom_posts'] as $custom_post) {
                        $dataToAdd = mapster_getOnlyValues($custom_post->ID);
                        if(isset($dataToAdd['data'])) {
                          ...
                          if(isset($dataToAdd['data'][$geographic_data_key]) && isset($dataToAdd['data']['mapster_map']['type'])) {
                            if(str_contains($dataToAdd['data'][$geographic_data_key]['type'], "Point")) {
                              $dataToAdd['data']['location'] = $dataToAdd['data'][$geographic_data_key];
                              array_push($minimized_location_data, $dataToAdd);
                            }
                            if(str_contains($dataToAdd['data'][$geographic_data_key]['type'], "Line")) {
                              $dataToAdd['data']['line'] = $dataToAdd['data'][$geographic_data_key];
                              array_push($minimized_location_data, $dataToAdd);
                            }
                            if(str_contains($dataToAdd['data'][$geographic_data_key]['type'], "Polygon")) {
                              $dataToAdd['data']['polygon'] = $dataToAdd['data'][$geographic_data_key];
                              array_push($minimized_location_data, $dataToAdd);
                            }
                            unset($dataToAdd['data'][$geographic_data_key]);
                          }
                          // $testdata = $dataToAdd;
                        }
                      }
                    }
                    unset($minimized_data['add_custom_posts']);
                  }
                }
            ...
            // $ch = curl_init();
            // curl_setopt($ch, CURLOPT_URL, "https://creatinglovein.business/wp-json/mapster-wp-maps/map?id=1625");
    ...
    if( mwm_fs()->can_use_premium_code() ) {
      ...
      if(mapster_can_be_looped($data)) {
        foreach($data as $feature) {
          if(isset($feature['data']['location_style']) && $feature['data']['location_style'] == "label") {
            $custom_properties = carbon_get_post_meta($feature["id"], 'mapster_custom_properties');
            $flattened_properties = array();
            foreach($custom_properties as $property) {
              $flattened_properties[$property['property_name']] = $property['property_value'];
            }
            if($feature['data']['label']['text_properties']['text-field']) {
              $feature['data']['label']['text_properties']['text-field'] = replace_text_with_property_or_acf($feature["id"], $flattened_properties, $feature['data']['label']['text_properties']['text-field']);
            }
          }
          if(isset($feature['data']['popup'])) {
            $custom_properties = carbon_get_post_meta($feature["id"], 'mapster_custom_properties');
            $flattened_properties = array();
            foreach($custom_properties as $property) {
              $flattened_properties[$property['property_name']] = $property['property_value'];
            }
            if($feature['data']['popup']['header_text']) {
              $feature['data']['popup']['header_text'] = replace_text_with_property_or_acf($feature["id"], $flattened_properties, $feature['data']['popup']['header_text']);
            }
            if($feature['data']['popup']['body_text']) {
              $feature['data']['popup']['body_text'] = replace_text_with_property_or_acf($feature["id"], $flattened_properties, $feature['data']['popup']['body_text']);
            }
            if($feature['data']['popup']['button_text']) {
              $feature['data']['popup']['button_text'] = replace_text_with_property_or_acf($feature["id"], $flattened_properties, $feature['data']['popup']['button_text']);
            }
            if($feature['data']['popup']['button_url']) {
              $feature['data']['popup']['button_url'] = replace_text_with_property_or_acf($feature["id"], $flattened_properties, $feature['data']['popup']['button_url']);
            }
            if($feature['data']['popup']['modal_details']) {
              $feature['data']['popup']['modal_details'] = replace_text_with_property_or_acf($feature["id"], $flattened_properties, $feature['data']['popup']['modal_details']);
            }
          }
          array_push($toReturn, $feature);
        }
      }
      return $toReturn;
    } else {
      ...
      if(!is_null($field) || $key == 'popup_style' || (isset($data[$key]) && !is_null($data[$key]))) {
        if(!isset($data[$key])) {
          ...
          if(mapster_can_be_looped($field)) {
            ...
            } else if(strval($data[$key]) !== strval($field)) {
              $toReturn[$key] = $data[$key];
            }
          }
        }
...
function mapster_getPropertyList($data) {
  ...
  if ( mwm_fs()->is__premium_only() ) {
		if( mwm_fs()->can_use_premium_code() ) {
      if(isset($data['id'])) {
        $custom_properties = carbon_get_post_meta($data["id"], 'mapster_custom_properties');
        foreach($custom_properties as $property) {
          $propertiesToReturn[$property['property_name']] = $property['property_value'];
        }
      }
    }
  }
  ...
  if(mapster_can_be_looped($terms)) {
    ...
    foreach($terms as $term) {
      ...
      if(is_plugin_active('sitepress-multilingual-cms/sitepress.php')) {
        $term_id = apply_filters( 'wpml_object_id', $term->term_id, "wp-map-category", true );
...
function mapster_update_wpml_post($duplicated_post_id, $new_post_id, $post_type) {
  if(is_plugin_active('sitepress-multilingual-cms/sitepress.php')) {
    ...
    if($duplicated_post_id) {
      $get_language_args = array('element_id' => $duplicated_post_id, 'element_type' => "post_" . $post_type );
