...
class Mapster_Wordpress_Maps_Admin {
	...
	public function enqueue_styles() {
  	...
  	if( $current_screen->id == "mapster-wp-map_page_wordpress-maps-settings" ) {
			wp_enqueue_style('mapster_map_settings', plugin_dir_url( __FILE__ ) . "css/mapster-wordpress-settings.css", array(), $this->version);
		}

  	if( $current_screen->id === "mapster-wp-popup" ) {
			wp_enqueue_style($this->plugin_name . '-popup', plugin_dir_url( __FILE__ ) . "css/dist/mwp-popup.css", array(), $this->version);
		}
	...
	public function enqueue_pro_styles__premium_only() {
  	$current_screen = get_current_screen();
		...
		if($settings_page_id) {
			$store_locator = get_field('pro_mwm_store_locator', $settings_page_id, true);
		}
		if( $current_screen->id == "mapster-wp-map_page_wordpress-maps-mass-edit" ) {
			wp_enqueue_style( 'mapster_wp_maps_mass_edit', plugin_dir_url( __FILE__ ) . 'css/pro/mapster-wordpress-maps-pro-mass-edit.css', array(), $this->version, 'all' );
		}
  	if( $current_screen->id == "mapster-wp-map" ) {
			if($store_locator) {
				wp_enqueue_style( 'mapster_wp_maps_store_locator', plugin_dir_url( __FILE__ ) . 'css/pro/mapster-wordpress-maps-pro-store-locator.css', array(), $this->version, 'all' );
			}
		}
	...
	public function enqueue_scripts( $hook_suffix ) {
		...
		wp_register_script('mapster_map_tutorial', plugin_dir_url( __FILE__ ) . "js/mapster-wordpress-maps-tutorial.js", array('quicktags', 'wp-pointer'), $this->version, true);
  	...
  	if( $current_screen->id == "mapster-wp-map" ) {
    	...
    	global $post;
			...
			if(MAPSTER_LOCAL_TESTING) {
				...
				$this->mapster_wordpress_maps_script_loading_dev($last_dependency, $map_provider, $settings_page_id, $model_3d_library, $elevation_chart_enabled, $store_locator_enabled, $injectedParams, $adminInjection);
			  ...
			  if ( mwm_fs()->is__premium_only() ) {
					if( mwm_fs()->can_use_premium_code() ) {
						$model_3d_library = get_field('model_3d_library', $post->ID);
						if($model_3d_library) {
							if($map_provider === 'maplibre' || $map_provider === 'custom-image') {
				...
				wp_register_script('mapster_map_admin_js', plugin_dir_url( __FILE__ ) . "js/dist/mwp-admin.js", array('jquery'), $this->version, true);
  	...
  	if( $current_screen->id == "mapster-wp-map_page_wordpress-maps-settings" ) {
			...
			if(MAPSTER_LOCAL_TESTING) {
				wp_register_script('turf', plugin_dir_url( __FILE__ ) . '../admin/js/vendor/turf.js', array('jquery'), $this->version, false );
				wp_register_script('csvtojson', plugin_dir_url( __FILE__ ) . '../admin/js/vendor/csvtojson.min.js', array('turf'), $this->version, false );
				wp_register_script('togeojson', plugin_dir_url( __FILE__ ) . '../admin/js/vendor/togeojson.js', array('csvtojson'), $this->version, false );
				wp_register_script('proj4', plugin_dir_url( __FILE__ ) . '../admin/js/vendor/proj4.js', array('togeojson'), $this->version, false );
				wp_register_script('shp', plugin_dir_url( __FILE__ ) . '../admin/js/vendor/shp.js', array('proj4'), $this->version, false );
				wp_register_script('mapster_map_settings_js', plugin_dir_url( __FILE__ ) . '../admin/js/mapster-wordpress-maps-settings.js', array('shp'), $this->version, false );
			} else {
				wp_register_script('mapster_map_settings_js', plugin_dir_url( __FILE__ ) . '../admin/js/dist/mwp-settings.js', array('jquery'), $this->version, false );
			}
  	...
  	if( $current_screen->id == "mapster-wp-popup" ) {
			wp_enqueue_script($this->plugin_name . '-popup', plugin_dir_url( __FILE__ ) . '/js/dist/mwp-popup.js', array('acf-input'), $this->version);
		}
		...
		if( $current_screen->id == "edit-mapster-wp-user-sub" || $current_screen->id == "edit-mapster-wp-popup" ||  $current_screen->id == "edit-mapster-wp-map" || $current_screen->id == "edit-mapster-wp-location" || $current_screen->id == "edit-mapster-wp-line" || $current_screen->id == "edit-mapster-wp-polygon") {
			wp_register_script($this->plugin_name . '-general', plugin_dir_url( __FILE__ ) . '/js/mapster-wordpress-maps-general.js', array('quicktags'), $this->version, true );
 ...
 * @since    1.0.0
 */
 public function mapster_wordpress_maps_script_loading_dev($last_dependency, $map_provider, $settings_page_id, $model_3d_library, $elevation_chart_enabled, $store_locator_enabled, $injectedParams, $adminInjection) {
		...
		wp_enqueue_script('mapster_map_sortable_js', plugin_dir_url( __FILE__ ) . "../admin/js/vendor/sortable.min.js", array($last_dependency), $this->version);
		$last_dependency = 'mapster_map_sortable_js';

		wp_enqueue_script('mapster_map_popper_js', plugin_dir_url( __FILE__ ) . "../admin/js/vendor/popper.min.js", array($last_dependency), $this->version);
		$last_dependency = 'mapster_map_popper_js';

		wp_enqueue_script('mapster_map_tippy_js', plugin_dir_url( __FILE__ ) . "../admin/js/vendor/tippy-bundle.umd.min.js", array($last_dependency), $this->version);
		$last_dependency = 'mapster_map_tippy_js';

		wp_register_script('mapster_map_admin_js', plugin_dir_url( __FILE__ ) . "../admin/js/mapster-wordpress-maps-admin.js", array($last_dependency), $this->version, true);
		wp_localize_script('mapster_map_admin_js', 'mapster_admin', $adminInjection);
		wp_enqueue_script('mapster_map_admin_js');
		...
		if($elevation_chart_enabled) {
			wp_enqueue_script('mapster_map_chart_js', plugin_dir_url( __FILE__ ) . "../admin/js/vendor/chart.min.js", array($last_dependency), $this->version);
	...
	public function enqueue_pro_scripts__premium_only() {
		...
		if( $current_screen->id == "mapster-wp-map_page_wordpress-maps-mass-edit" ) {
			wp_register_script( 'mapster_map_pro_mass_edit_js', plugin_dir_url( __FILE__ ) . '/js/pro/mapster-wordpress-maps-pro-mass-edit.js', array('acf-input'), $this->version, false);
			wp_localize_script( 'mapster_map_pro_mass_edit_js', 'mapster_mass_edit', array(
				'strings' => $i18n->get_mapster_strings()['mass_edit_js'],
				'rest_url' => get_rest_url(),
				'qd' => $this->mapster_get_rest_url_delimiter(),
				'nonce' => wp_create_nonce( 'wp_rest' )
			));
			wp_enqueue_script('mapster_map_pro_mass_edit_js');
		}

		if( $current_screen->id == "mapster-wp-map_page_wordpress-maps-settings" ) {
			wp_enqueue_script( 'mapster_map_pro_settings_js', plugin_dir_url( __FILE__ ) . '/js/pro/mapster-wordpress-maps-pro-settings.js', array('mapster_map_settings_js'), $this->version, false);
		}
	}
	...
	public function create_mapster_wp_maps_post_types()
	{
	    ...
	    register_taxonomy(
	        ...
	        array(
	            'hierarchical' => true,
							'show_in_rest' => true,
	            'label' => $i18n->get_mapster_strings()['admin']['Map Categories'], // display name
	            'query_var' => true,
			...
			if($settings_page_id) {
				...
				if($user_submission) {
				  if ( mwm_fs()->is__premium_only() ) {
						if( mwm_fs()->can_use_premium_code() ) {
							register_post_type('mapster-wp-user-sub',
									array(
											'labels' => array(
													'name' => $i18n->get_mapster_strings()['admin']['User Submissions'],
													'menu_name' => $i18n->get_mapster_strings()['admin']['User Submissions'],
													'singular_name' => $i18n->get_mapster_strings()['admin']['User Submission'],
													'add_new' => $i18n->get_mapster_strings()['admin']['Add New'],
													'add_new_item' => $i18n->get_mapster_strings()['admin']['Add New User Submission'],
													'edit' => $i18n->get_mapster_strings()['admin']['Edit'],
													'edit_item' => $i18n->get_mapster_strings()['admin']['Edit User Submission'],
													'new_item' => $i18n->get_mapster_strings()['admin']['New User Submission'],
													'view' => $i18n->get_mapster_strings()['admin']['View'],
													'view_item' => $i18n->get_mapster_strings()['admin']['View User Submission'],
													'search_items' => $i18n->get_mapster_strings()['admin']['Search User Submission'],
													'not_found' => $i18n->get_mapster_strings()['admin']['No User Submission found'],
													'not_found_in_trash' => $i18n->get_mapster_strings()['admin']['No User Submission found in Trash'],
													'parent' => $i18n->get_mapster_strings()['admin']['Parent User Submission'],
											),
											'public' => true,
				        		  "publicly_queryable"  => $public_pages['user_submissions'] ? true : false,
						   				'exclude_from_search' => true,
											'show_in_rest' => true,
											'supports' => array('title','custom-fields'),
											'taxonomies' => array('wp-map-category'),
				    					'rewrite' => isset($permalinks['user_submissions']) ? array(
												'slug' => $permalinks['user_submissions']
											) : array(),
											'has_archive' => true,
				    					'show_in_menu' => 'edit.php?post_type=mapster-wp-map'
									)
  ...
  function set_custom_mapster_map_features_column($columns) {
	...
	public function add_mapster_wp_map_metabox() {
		$i18n = new Mapster_Wordpress_Maps_i18n();
		add_meta_box( 'mapster-wp-maps-preview', $i18n->get_mapster_strings()['admin']['Map Preview'], 'my_meta_box_callback',
	...
	public function mapster_wp_maps_settings_menu()
	{
		  ...
		  if ( mwm_fs()->is__premium_only() ) {
				if( mwm_fs()->can_use_premium_code() ) {
					add_submenu_page('edit.php?post_type=mapster-wp-map', $i18n->get_mapster_strings()['mass_edit']['Mass Edit Button'], $i18n->get_mapster_strings()['mass_edit']['Mass Edit Button'], 'manage_options', 'wordpress-maps-mass-edit', function () {
							include 'partials/mapster-wordpress-maps-mass-edit.php';
					});
				}
			}
			add_submenu_page('edit.php?post_type=mapster-wp-map', $i18n->get_mapster_strings()['admin']['Categories'], $i18n->get_mapster_strings()['admin']['Categories'], 'manage_options', 'edit-tags.php?taxonomy=wp-map-category&post_type=mapster-wp-map');
			add_submenu_page('edit.php?post_type=mapster-wp-map', $i18n->get_mapster_strings()['admin']['Settings'], $i18n->get_mapster_strings()['admin']['Settings'], 'manage_options', 'wordpress-maps-settings', function () {
  	...
  	if( strpos( $current_screen->id, "mapster" ) !== false || strpos( $current_screen->post_type, "mapster" ) !== false) {
			?>
				<div class="mapster-admin-toolbar">
					<h2><i class="acf-tab-icon dashicons dashicons-location-alt"></i> <?php echo $i18n->get_mapster_strings()['admin']['Top Menu Header']; ?></h2>
					<a class="acf-tab <?php echo $current_screen->id == 'edit-mapster-wp-map' || $current_screen->id === 'mapster-wp-map' ? "is-active" : "" ?>" href="edit.php?post_type=mapster-wp-map"><?php echo $i18n->get_mapster_strings()['admin']['Maps']; ?></a>
					<a class="acf-tab <?php echo $current_screen->post_type == 'mapster-wp-location' ? "is-active" : "" ?>" href="edit.php?post_type=mapster-wp-location"><?php echo $i18n->get_mapster_strings()['admin']['Locations']; ?></a>
					<a class="acf-tab <?php echo $current_screen->post_type == 'mapster-wp-line' ? "is-active" : "" ?>" href="edit.php?post_type=mapster-wp-line"><?php echo $i18n->get_mapster_strings()['admin']['Lines']; ?></a>
					<a class="acf-tab <?php echo $current_screen->post_type == 'mapster-wp-polygon' ? "is-active" : "" ?>" href="edit.php?post_type=mapster-wp-polygon"><?php echo $i18n->get_mapster_strings()['admin']['Polygons']; ?></a>
					<a class="acf-tab <?php echo $current_screen->post_type == 'mapster-wp-popup' ? "is-active" : "" ?>" href="edit.php?post_type=mapster-wp-popup"><?php echo $i18n->get_mapster_strings()['admin']['Popup Templates']; ?></a>
					<?php
		        if ( mwm_fs()->can_use_premium_code() ) {
							$settings_page_id = get_option('mapster_settings_page');
							if($settings_page_id) {
								$user_submission = get_field('pro_mwm_user_submission', $settings_page_id);
								if($user_submission) { ?>
									<a class="acf-tab <?php echo $current_screen->post_type == 'mapster-wp-user-sub' ? "is-active" : "" ?>" href="edit.php?post_type=mapster-wp-user-sub"><?php echo $i18n->get_mapster_strings()['admin']['User Submission']; ?></a>
								<?php }
							}
						}
					?>
					<?php
		        if ( mwm_fs()->can_use_premium_code() ) { ?>
							<a class="acf-tab <?php echo $current_screen->id == 'mapster-wp-map_page_wordpress-maps-mass-edit' ? "is-active" : "" ?>" href="edit.php?post_type=mapster-wp-map&page=wordpress-maps-mass-edit"><?php echo $i18n->get_mapster_strings()['admin']['Mass Edit']; ?></a>
					<?php } ?>
					<a class="acf-tab <?php echo $current_screen->id == 'edit-wp-map-category' ? "is-active" : "" ?>" href="edit-tags.php?taxonomy=wp-map-category&post_type=mapster-wp-map"><?php echo $i18n->get_mapster_strings()['admin']['Categories']; ?></a>
					<a class="acf-tab <?php echo $current_screen->id == 'mapster-wp-map_page_wordpress-maps-settings' || $current_screen->id == 'mapster-wp-map_page_wordpress-maps-settings-pricing' || $current_screen->id == 'mapster-wp-map_page_wordpress-maps-settings-account' ? "is-active" : "" ?>" href="edit.php?post_type=mapster-wp-map&page=wordpress-maps-settings"><?php echo $i18n->get_mapster_strings()['admin']['Settings']; ?></a>
					<?php if ( !mwm_fs()->can_use_premium_code() ) { ?>
						<a target="_blank" href="https://wpmaps.mapster.me/pro" class="btn-upgrade">
							...
							<p><?php echo $i18n->get_mapster_strings()['admin']['Download Pro']; ?></p>
						</a>
					<?php } ?>
				</div>
			<?php
	...
	/**
	 * Register default user submission page
	 *
	 * @since    1.0.0
	 */
	public function add_mapster_wp_maps_user_submission_page__premium_only( ) {
			// Create default page
			$settings_page_id = get_option('mapster_settings_page');
			if($settings_page_id) {
				 $user_submission = get_field('pro_mwm_user_submission', $settings_page_id);
				 if($user_submission) {
				    $query = new WP_Query( array(
				     ...
				     'meta_value' => 'mapster-submission-template.php'
						...
						if (!$query->have_posts()) {
			 				$submission_template = wp_insert_post(array(
			 					'post_type' => "page",
			 					'post_title' => "User Submission Template",
			 					'post_status' => "publish",
			 					'page_template' => 'mapster-submission-template.php'
			 				));
						}
				 }
			}
	...
	/**
	 * Create Mapster Wordpress Maps database table
	 *
	 * @since    1.0.0
	 */
	function mapster_create_db_table__premium_only() {

		$settings_page_id = get_option('mapster_settings_page');
		if($settings_page_id) {
			$spatial_api_enabled = get_field('pro_spatial_api', $settings_page_id);
			if($spatial_api_enabled) {
				if(!get_option('mapster_created_spatial_db')) {
					...
					// Populate Spatial DB
					$post_count = 0;
          $post_count += wp_count_posts('mapster-wp-location')->publish;
          $post_count += wp_count_posts('mapster-wp-line')->publish;
          $post_count += wp_count_posts('mapster-wp-polygon')->publish;

          $posts_per_query = 250;
          $full_pages = floor($post_count / $posts_per_query);
          $current_post_count = 0;

          $insert_arrays = array();

          for($i = 0; $i < $full_pages + 1; $i++) {
        		$args = array(
        			'post_type' => array('mapster-wp-location', 'mapster-wp-line', 'mapster-wp-polygon'),
        			'post_status' => 'publish',
        			'posts_per_page' => $posts_per_query,
              'offset' => $i * $posts_per_query
        		);
                ...
                if($field_type) {
          				$coordinate_data = get_field($field_type, get_the_ID(), true);
                  if($coordinate_data) {
            				$decoded_coordinate_data = json_decode($coordinate_data);
            				$coordinate_geojson = geoPHP::load(json_encode($decoded_coordinate_data->features[0]), 'json');
            				array_push($insert_arrays, array(
            					'post_id' => get_the_ID(),
            					'coordinates' => $coordinate_geojson->out('wkt')
            				));
                  }
                }
        			endwhile;
          ...
          $posts_looped = array();
      		foreach($insert_arrays as $item) {
            $post_id = $item['post_id'];
            $coordinates = $item['coordinates'];
      			$sql = "
              INSERT INTO $table_name (post_id, coordinates) (
                  SELECT '" . $post_id . "', ST_GeomFromText('" . $coordinates . "', 3857)
                    WHERE NOT EXISTS (
                      SELECT 1
                        FROM $table_name
            ...
            ";
      			$result = $wpdb->query( $sql );
					...
					update_option( 'mapster_created_spatial_db', true );
				}
			}
		}
	}
	...
	/**
	 * Update Mapster database entry
	 *
	 * @since    1.0.0
	 */
	 function mapster_update_custom_db__premium_only($post_id) {

		 $post_type = get_post_type($post_id);
     if ($post_type == 'mapster-wp-location' || $post_type == 'mapster-wp-line' || $post_type == 'mapster-wp-polygon') {
			 ...
			 if($settings_page_id) {
				 ...
				 if($spatial_api_enabled) {
					 ...
					 $coordinate_data = get_field($field_type, $post_id, true);
					 if($coordinate_data) {
						 $decoded_coordinate_data = json_decode($coordinate_data);
						 if($decoded_coordinate_data->features[0]) {
							 $coordinate_geojson = geoPHP::load(json_encode($decoded_coordinate_data->features[0]), 'json');
							 $coordinates = $coordinate_geojson->out('wkt');
							 $sql = "
								 UPDATE $table_name
								 SET coordinates = ST_GeomFromText('" . $coordinates . "', 3857)
								 WHERE post_id = '" . $post_id . "'
							 ";
							 $result = $wpdb->query( $sql );
							 if($result == 0) {
								 $sql = "
									 INSERT INTO $table_name (post_id, coordinates)
									 VALUES('" . $post_id . "', ST_GeomFromText('" . $coordinates . "', 3857))
								 ";
								 $result = $wpdb->query( $sql );
							 }
						 	 return;
	...
	/**
	 * Export User Submissions as CSVs
	 *
	 * @since    1.0.0
	 */
	function mapster_add_csv_export_button__premium_only($mime_types){
		global $current_screen;
    if ('mapster-wp-user-sub' != $current_screen->post_type) {
        return;
    }
		$i18n = new Mapster_Wordpress_Maps_i18n();
		$translation_strings = $i18n->get_mapster_strings()['user_submission'];
    ?>
        <script type="text/javascript">
					jQuery(function () {
						let csvExportText = "<?php echo $translation_strings['CSV Export Button']; ?>";
						jQuery('hr.wp-header-end').before(`<a id='mapster-csv-export-modal-open' name='CSV Exporter' href='#TB_inline?width=100&amp;inlineId=mapster-csv-export-modal' class='thickbox add-new-h2'>${csvExportText}</a>`);
					});
				</script>

				<div style="width: 50%; padding-left: 20px;">
					<div id="mapster-csv-export-modal" style="display: none;">
						<p><?php echo $translation_strings['CSV Export Description']; ?></p>
						<div style="margin-top: 10px;">
							<button id="mapster-csv-export-select-all" class="button button-small"><?php echo $translation_strings['CSV Export Select All']; ?></button>
							<button id="mapster-csv-export-select-none" class="button button-small"><?php echo $translation_strings['CSV Export Select None']; ?></button>
						</div>
						<div id="mapster-csv-export-modal-content"></div>
						<button id="mapster-wp-user-sub-save-options" class="button button-large" style="margin-right: 10px;"><?php echo $translation_strings['CSV Export Save Options']; ?></button>
						<button id="mapster-wp-user-sub-export" class="button button-primary button-large"><?php echo $translation_strings['CSV Export Download']; ?></button>
					</div>
				</div>
    <?php
	}
	 ...
	 * @since    1.0.0
	 */
	function mapster_allow_svg_upload__premium_only($mime_types){
	 ...
	 * @since    1.0.0
	 */
	function mapster_allow_gltf_upload__premium_only($mime_types){
	...
	/**
	 * Ensure GLB is recognized as such
	 *
	 * @since    1.0.0
	 */
	function mapster_recognize_gltf_upload__premium_only($data, $file, $filename, $mime_types, $real_mime_type){
	 ...
	 */
	function mapster_submission_add_template_to_select__premium_only( $post_templates, $wp_theme, $post, $post_type ) {
	    $post_templates['mapster-submission-template.php'] = __('Mapster User Submission');
	 ...
	 */
	function mapster_submission_load_plugin_template__premium_only( $template ) {

	    if(  get_page_template_slug() === 'mapster-submission-template.php' ) {

	        if ( $theme_file = locate_template( array( 'mapster-submission-template.php' ) ) ) {
	            $template = $theme_file;
	        } else {
	            $template = plugin_dir_path( __FILE__ ) . 'includes/mapster-submission-template.php';
	        }
	    }
	...
	/**
	 * Add shortcode column to Location, Line, Polygon types
	 *
	 * @since    1.0.0
	 */
  function set_custom_feature_column__premium_only($columns) {
      $columns['id'] = __( 'ID', 'mapster-wp-maps' );
      return $columns;
  }
	 ...
	 * @since    1.0.0
	 */
	function custom_feature_id_column__premium_only( $column, $post_id ) {
      switch ( $column ) {
        case 'id' :
          echo $post_id;
          break;
	...
	/**
	 * When saving a new post, notify admins
	 *
	 * @since    1.0.0
	 */
	// function mapster_notify_new_post__premium_only( $post_id ) {
    // if( $post_id != 'new' ) {
    //   return $post_id;
    // } else {
		// 	 $settings_page_id = get_option('mapster_settings_page');
		// 	 if($settings_page_id) {
		// 		 $users_to_notify = get_field('pro_user_submission_options_notify_users', $settings_page_id);
		// 		 foreach($users_to_notify as $user_id) {
		// 			  $user_info = get_userdata($user_id);
		// 			 	$to = $user_info->user_email;
		// 				$subject = "New Geography Submitted to " . get_bloginfo('name');
		// 				$body = "A new geography feature post was submitted to " . get_bloginfo('name') . " at " . current_time('d/m/y H:i:s') . ". Head over and take a look!";
  ...
  // }
	...
	/**
	 * Registering repeater fields to handle custom properties
	 *
	 * @since    1.0.0
	 */
	public function mapster_add_repeater_property_fields__premium_only() {
		$types = ["mapster-wp-location", "mapster-wp-line", "mapster-wp-polygon"];
		foreach($types as $type) {
			Carbon_Fields\Container::make( 'post_meta', 'Custom Properties' )
		    ->where( 'post_type', '=', $type )
		    ->add_fields( array(
					Carbon_Fields\Field::make( 'complex', 'mapster_custom_properties' )
						->add_fields( array(
								Carbon_Fields\Field::make( 'text', 'property_name' )
									->set_width( 50 ),
								Carbon_Fields\Field::make( 'text', 'property_value' )
									->set_width( 50 ),
						) )
		    ));
		}
	...
	/**
	 * Modify fields returned for mass edit page
	 *
	 * @since    1.0.0
	 */
	function mapster_mass_edit_acf_fields__premium_only($field) {
		if(is_admin()) {
			$current_screen = get_current_screen();
	 		if( $current_screen->id !== "mapster-wp-map_page_wordpress-maps-mass-edit" ) {
				return $field;
	...
	/**
	 * Show appropriate styling and popup details if a custom field is included
	 *
	 * @since    1.0.0
	 */
	function mapster_acf_group_field_custom_posts__premium_only($groups) {
		$get_current_screen = get_current_screen();
    $post_type = $get_current_screen->post_type;
		$new_field_groups = array();
		foreach($groups as $group) {
			array_push($new_field_groups, $group);
		}
		// var_dump("HEY");
		// var_dump($this->questionable_has_mapster_map_acf);
		// $field_groups = acf_get_field_groups(array('post_type' => $post_type));
		// var_dump($field_groups);
		// $this_post_has_mapster_field = false;
		// if($post_type !== "mapster-wp-location" && $post_type !== "mapster-wp-line" && $post_type !== "mapster-wp-polygon" && $post_type !== "mapster-wp-user-sub") {
		// 	foreach($groups as $group) {
		...
		// If it's referenced at all in the visibility options, then load it
		// Then on the frontend, move the fields and hide them until a button is pressed
		// Then that expands them out from inside the existing field

		// $mapster_map_key = 'group_6163732e0426e';
		//
		// 	foreach($groups as $group) {
		// 		if($group['key'] == $location_field_group_key) {
		// 			$new_location_rules = $group['location'];
		// 			array_push($new_location_rules, array(
		// 				array(
		// 						'param' => 'post_type',
		// 						'operator' => '==',
		// 						'value' => 'post',
		// 				)
		// 			));
		// 			$group['location'] = $new_location_rules;
